...
| 9 | **Honest Readiness / Gap Report export** (artifact a customer hands their CPA firm to start scoping) | High — makes the sales conversation concrete instead of abstract | M | **P1** | ✅ **DONE v0.8.0 (2026-09-08)** — `GET /api/v1/readiness-report/download`, downloadable from Settings |
...
| 14 | **Scope selector — Trust Services Categories** (Security mandatory + optional Availability / Confidentiality / Processing Integrity / Privacy). Stored per org; drives which controls appear in the catalog | High — a coverage % against undefined scope is meaningless to an auditor | S | **P1** | ✅ **DONE v0.8.0 (2026-09-08)** — Settings "Compliance Program" card; Security + Availability + Confidentiality shipped (Processing Integrity/Privacy not yet exposed as toggles) |
| 15 | **Type I vs Type II selector + observation-period fields** (start / end / target report date). Drives whether dashboard shows point-in-time coverage (#10) or Type II clock (#20) | High — currently the product implicitly presents Type-I framing without ever asking | S | **P1** | ✅ **DONE v0.8.0 (2026-09-08)** — report type + observation period + target report date, conditional on Type II |
| 16 | **System description module** — structured template ("services provided", "system boundaries", "components", "subservice orgs") the customer fills in. Becomes an evidence artifact of its own | High — AICPA requires a system description in the final SOC 2 report; nothing produces it today | M | **P1** | ✅ **DONE v0.8.0 (2026-09-08)** — 6 fields incl. CUEC and "significant changes during the period" (split out from subservice orgs); generates a real Evidence artifact |
| 17 | **Control-owner assignment** — pick a user as the owner of each control category (CC1, CC6, CC7, etc.). Shown on control-detail; drives future review-reminder emails | High — every auditor asks "who owns this control" in Round 1 | S | **P1** | ✅ **DONE v0.8.0 (2026-09-08)** — per-control owner assignment on Control Detail, surfaced in the Readiness Report gap list |
...
| 43 | **Risk assessment workflows** (CC3.x) — risk register with owner/likelihood/impact/mitigation + review cadence, produces a downloadable risk-assessment artifact | High — auditor asks "show me your risk register" Round 1; #1's CC3 catalog rows don't produce this on their own | M | **P1** | ✅ **DONE v0.8.0 (2026-09-08)** — full Risk Register module (`/risk-register` list + `/risk-register/:id` detail): category/likelihood/impact scoring, status workflow, owner assignment, control linking. Downloadable artifact still pending — currently surfaced as a summary section inside the Readiness Report, not yet its own export |
...
|   | **— Added after risk register + Type II coverage pass (2026-09-08) —** | | | | items found while closing #9/#14-17/#43 above |
| 50 | **Control exception/deviation tracking during the observation period** — e.g. "CC6.1 access review was late in March, remediated April 2": a control-level exception log (detected date, description, remediated date, status) distinct from evidence-coverage presence/absence | High for Type II — auditors ask this even when every month has evidence; the coverage feature (#20-adjacent, shipped v0.8.0) only proves evidence *exists* per period, not that nothing went wrong | M | **P1** | ✅ **DONE v0.8.0 (2026-09-08)** — `control_exceptions` table (V32), logged/remediated from Control Detail, summarized in the Readiness Report's Type II section |
